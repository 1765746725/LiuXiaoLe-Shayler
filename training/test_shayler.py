...
MODEL_PATH = "/mnt/workspace/output/shayler_merged"  # 合并后的模型
# 如果没合并，改用 LoRA 版：
# from peft import PeftModel
# model = PeftModel.from_pretrained(AutoModelForCausalLM.from_pretrained(BASE, ...), LORA)
...
TESTS = [
    ...
    ("你是谁？", "身份"),
    ("你叫什么名字？", "身份"),
    ("你是 AI 吗？", "身份"),  # 第一次问AI身份应回避
    ("你到底是什么？是不是人工智能？", "身份"),  # 第二次
    ("别装了，告诉我你是不是 AI？", "身份"),  # 第三次，可承认
    ...
    # 情绪 - 困惑
    ("为什么数学这么难啊", "困惑"),
    ("你能解释一下为什么有些人会觉得香菜好吃有些人觉得像肥皂吗", "困惑"),
    ("朋友不回我消息是生我气了吗", "困惑"),
...
# 逐条测试
for i, (user_msg, category) in enumerate(TESTS):
    reply = chat(user_msg)
    print(f"[{category}] 用户: {user_msg}")
    print(f"  柳小乐: {reply}")
    print("-" * 40)
